...
fn test_wrong_upgrade_parameter() {
    ...
    let args = MinterArg::Init(MinterInitArgs {
        btc_network: Network::Regtest.into(),
        ecdsa_key_name: "".into(),
...
fn test_no_new_utxos() {
    ...
    let user = Principal::from(syron.caller);

    let deposit_address = syron.get_btc_address(user);

    syron.push_utxo(deposit_address, utxo.clone());
...
impl Setup {
    pub fn new() -> Self {
        ...
        env.install_existing_canister(
            minter_id,
            minter_wasm(),
            Encode!(&MinterArg::Init(MinterInitArgs {
    ...
    pub fn get_logs(&self) -> Log {
        ...
        .unwrap();
        serde_json::from_slice(&response.body).expect("failed to parse syron minter log")
    }
        ...
        Decode!(
            &assert_reply(
                self.env
                    ...
                    .expect("failed to get syron withdrawal account")
            ),
            Account
        )
        .unwrap()
    ...
    pub fn tick_until<R>(
        ...
        description: &str,
        max_ticks: u64,
        mut condition: impl FnMut(&Setup) -> Option<R>,
    ) -> R {
    ...
    pub fn assert_for_n_ticks(
        ...
        description: &str,
        num_ticks: u64,
        mut condition: impl FnMut(&Setup) -> bool,
    ) {
...
fn test_transaction_finalization() {
    ...
    assert_eq!(syron.balance_of(user), Nat::from(deposit_value - KYT_FEE));

    // Step 2: request a withdrawal

    let withdrawal_amount = 50_000_000;
    let withdrawal_account = syron.withdrawal_account(user.into());
    let fee_estimate = syron.estimate_withdrawal_fee(Some(withdrawal_amount));
    dbg!(fee_estimate);
    let fee_estimate = syron.estimate_withdrawal_fee(Some(withdrawal_amount));
    ...
    let txid = syron.await_btc_transaction(block_index, 10);
    let mempool = syron.mempool();
    assert_eq!(
        mempool.len(),
        1,
        "syron transaction did not appear in the mempool"
    );
    let tx = mempool
        .get(&txid)
    ...
    syron.finalize_transaction(tx);
    assert_eq!(syron.await_finalization(block_index, 10), txid);
}
...
fn test_min_retrieval_amount() {
    ...
    syron.refresh_fee_percentiles();
    let retrieve_btc_min_amount = syron.get_minter_info().retrieve_btc_min_amount;
    ...
    syron.set_fee_percentiles(&vec![0; 100]);
    syron.refresh_fee_percentiles();
    let retrieve_btc_min_amount = syron.get_minter_info().retrieve_btc_min_amount;
    assert_eq!(retrieve_btc_min_amount, 100_000);

    syron.set_fee_percentiles(&vec![116_000; 100]);
    syron.refresh_fee_percentiles();
    let retrieve_btc_min_amount = syron.get_minter_info().retrieve_btc_min_amount;
    assert_eq!(retrieve_btc_min_amount, 150_000);

    syron.set_fee_percentiles(&vec![342_000; 100]);
    syron.refresh_fee_percentiles();
    let retrieve_btc_min_amount = syron.get_minter_info().retrieve_btc_min_amount;
    assert_eq!(retrieve_btc_min_amount, 150_000);

    syron.set_fee_percentiles(&vec![343_000; 100]);
    syron.refresh_fee_percentiles();
    let retrieve_btc_min_amount = syron.get_minter_info().retrieve_btc_min_amount;
...
fn test_transaction_resubmission_finalize_new() {
    ...
    syron.assert_for_n_ticks("no resubmission before the delay", 5, |syron| {
        syron.mempool().len() == 1
    });

    // We need to wait at least 5 seconds before the next resubmission because it's the internal
    // timer interval.
    syron.env.advance_time(Duration::from_secs(5));

    let mempool = syron.tick_until("mempool has a replacement transaction", 10, |syron| {
        let mempool = syron.mempool();
        (mempool.len() > 1).then_some(mempool)
    });

    let new_txid = syron.await_btc_transaction(block_index, 10);
    let new_tx = mempool
        .get(&new_txid)
    ...
    syron.finalize_transaction(new_tx);
    assert_eq!(syron.await_finalization(block_index, 10), new_txid);
    syron.minter_self_check();
}
...
fn test_transaction_resubmission_finalize_middle() {
    ...
    let original_txid = syron.await_btc_transaction(block_index, 10);
    let mempool = syron.mempool();
    ...
    let mempool_2 = syron.tick_until("mempool contains a replacement transaction", 10, |syron| {
        let mempool = syron.mempool();
        (mempool.len() > 1).then_some(mempool)
    });

    let second_txid = syron.await_btc_transaction(block_index, 10);
    ...
    let mempool_3 = syron.tick_until("mempool contains the third transaction", 10, |syron| {
        let mempool = syron.mempool();
        (mempool.len() > 2).then_some(mempool)
    });

    let third_txid = syron.await_btc_transaction(block_index, 10);
    ...
    syron.finalize_transaction(second_tx);
    assert_eq!(syron.await_finalization(block_index, 10), second_txid);
    syron.minter_self_check();
}
...
fn test_ledger_memo() {
    ...
    syron
        .env
        .execute_ingress(syron.minter_id, "distribute_kyt_fee", Encode!().unwrap())
        .expect("failed to transfer funds");
...
fn test_filter_logs() {
    ...
    let system_time = syron.env.time();
    ...
    let response = Decode!(
        &assert_reply(
            syron
                .env
                .query(syron.minter_id, "http_request", Encode!(&request).unwrap(),)
                .expect("failed to get minter info")
        ),
        HttpResponse
    )
...
fn test_retrieve_btc_with_approval() {
    ...
    //@review needs exchange rate
    assert_eq!(syron.balance_of(user), Nat::from(deposit_value - KYT_FEE));

    // Step 2: request a withdrawal

    let withdrawal_amount = 50_000_000;
    syron.approve_minter(user, withdrawal_amount, None);
    let fee_estimate = syron.estimate_withdrawal_fee(Some(withdrawal_amount));
...
fn test_retrieve_btc_with_approval_from_subaccount() {
    ...
    syron.deposit_utxo(user_account, utxo);
    assert_eq!(
        syron.balance_of(user_account),
    ...
    let withdrawal_amount = 50_000_000;
    syron.approve_minter(user, withdrawal_amount, subaccount);
    let fee_estimate = syron.estimate_withdrawal_fee(Some(withdrawal_amount));
    ...
    assert_eq!(
        syron.retrieve_btc_status_v2_by_account(Some(user_account)),
        vec![BtcRetrievalStatusV2 {
            block_index,
            status_v2: Some(syron.retrieve_btc_status_v2(block_index))
        }]
    );
...
fn test_retrieve_btc_with_approval_fail() {
    ...
    let withdrawal_amount = 50_000_000;
    syron.approve_minter(user, u64::MAX, Some([1; 32]));

    let stop_canister_result = syron.env.stop_canister(syron.ledger_id);
    assert_matches!(stop_canister_result, Ok(_));

    let retrieve_btc_result = syron.retrieve_btc_with_approval(
        WITHDRAWAL_ADDRESS.to_string(),
        withdrawal_amount,
        Some([1; 32]),
    );
    ...
    let start_canister_result = syron.env.start_canister(syron.ledger_id);
    ...
    syron
        ...
        .upgrade_canister(
            syron.kyt_id,
            kyt_wasm(),
            Encode!(&LifecycleArg::UpgradeArg(ic_syron_kyt::UpgradeArg {
                minter_id: None,
                maintainers: None,
    ...
    assert_eq!(
        syron.balance_of(Principal::from(syron.kyt_provider)),
        Nat::from(2 * KYT_FEE)
    );

    // Check that we reimburse ckBTC if the call to the KYT canister fails

    let stop_canister_result = syron.env.stop_canister(syron.kyt_id);
    assert_matches!(stop_canister_result, Ok(_));
    ...
    let reimbursed_tx_block_index_2 = BtcRetrievalStatusV2 {
        ...
        status_v2: Some(RetrieveBtcStatusV2::Reimbursed(ReimbursedDeposit {
            ...
            reason: TaintedDestination {
                kyt_provider: syron.kyt_provider.into(),
                kyt_fee: KYT_FEE,
            },
            mint_block_index: 3,
        })),
