...
/// The minimum number of pending request in the queue before we try to make a batch transaction. @review (alpha)
pub const MIN_PENDING_REQUESTS: usize = 1;
...
/// The maximum memo size of a transaction on the ledger.
/// The minter requires at least 69 bytes, so let's choose 80
/// to have some room for future modifications.
pub const LEDGER_MEMO_SIZE: u16 = 80;
...
struct SignTxRequest {
    ...
    utxos: Vec<Utxo>,
    /// The list of UTXOs we use as transaction inputs to cover gas & output values if needed
    sats_utxos: Vec<Utxo>,
}
...
fn undo_sign_request(requests: Vec<state::RetrieveBtcRequest>, runes_utxos: Vec<Utxo>, sats_utxos: Vec<Utxo>) {
    state::mutate_state(|s| {
        for utxo in runes_utxos {
            assert!(s.available_utxos.insert(utxo));
        }
        for utxo in sats_utxos {
            assert!(s.available_sats_utxos.insert(utxo));
        }
...
async fn submit_pending_requests() {
    ...
    let maybe_sign_request = state::mutate_state(|s| {
        ...
        // @dev Create fresh outputs for each attempt to prevent treasury fee accumulation
        ...
        // Clone the UTXO sets to avoid double mutable borrow
        let mut available_utxos = s.available_utxos.clone();
        let mut available_sats_utxos = s.available_sats_utxos.clone();

        match build_unsigned_transaction(
            &mut available_utxos,
            &mut available_sats_utxos,
            outputs,
            main_address,
            fee_millisatoshi_per_vbyte,
            s.dao_addr[1].clone(),
        ) {
            Ok((unsigned_tx, change_output, utxos, sats_utxos)) => {
                // Update the state with the modified UTXO sets
                s.available_utxos = available_utxos;
                s.available_sats_utxos = available_sats_utxos;
                ...
                Some(SignTxRequest {
                    ...
                    ecdsa_public_key: s.ecdsa_public_key.clone().expect("ECDSAPublicKey is None"),
                    change_output,
            ...
            Err(BuildTxError::NotEnoughRunes) => {
                log!(P0,
                    "[submit_pending_requests]: not enough runes UTXOs to unsigned transaction for requests at block indexes [{}]",
                    batch.iter().map(|req| req.block_index.to_string()).collect::<Vec<_>>().join(",")
                );
    ...
    if let Some(req) = maybe_sign_request {
        ...
        let requests_guard = guard((req.requests, req.utxos, req.sats_utxos), |(reqs, runes_utxos, sats_utxos)| {
            undo_sign_request(reqs, runes_utxos, sats_utxos);
        });
            ...
            Ok(signed_tx) => {
                ...
                match management::send_transaction(&signed_tx, req.network).await {
                    Ok(()) => {
                        ...
                        // successfully.
                        let (requests, used_utxos, used_sats_utxos) = ScopeGuard::into_inner(requests_guard);
...
async fn finalize_requests() {
    ...
    let (dao_addr, btc_network, ecdsa_public_key) = state::read_state(|s| (s.dao_addr.clone(), s.btc_network, s.ecdsa_public_key.clone().expect("ECDSAPublicKey is None")));
    let main_address =
    //address::account_to_bitcoin_address(&ecdsa_public_key, &main_account, "");
    &dao_addr[2]; // runes minter

    // runes minter subaccount
    let subaccount = compute_subaccount(0, &main_address.display(btc_network));
    ...
    for (old_txid, submitted_tx) in maybe_finalized_transactions {
        let mut submitted_runes_utxos: BTreeSet<_> = submitted_tx.used_runes_utxos.iter().cloned().collect();
        let mut submitted_sats_utxos: BTreeSet<_> = submitted_tx.used_sats_utxos.iter().cloned().collect();
        ...
        let (unsigned_tx, change_output, used_runes_utxos, used_sats_utxos) = match build_unsigned_transaction(
            &mut submitted_runes_utxos,
            &mut submitted_sats_utxos,
            outputs,
            main_address.clone(),
            tx_fee_per_vbyte,
        ...
        assert!(
            submitted_runes_utxos.is_empty(),
            "build_unsigned_transaction didn't use all inputs"
        );
        assert_eq!(used_runes_utxos.len(), submitted_tx.used_runes_utxos.len());
        assert_eq!(used_sats_utxos.len(), submitted_tx.used_sats_utxos.len());
...
fn greedy(target: u64, available_utxos: &mut BTreeSet<Utxo>) -> Vec<Utxo> {
    ...
    if !solution.is_empty() && solution.iter().map(|u| u.value).sum::<u64>() < target {
        // This should never happen due to the algorithm logic, but validate in production
        ic_cdk::println!("BUG: Solution total value {} is less than target {}", 
...
pub enum BuildTxError {
    ...
    NotEnoughFunds,
    /// The minter does not have enough Runes to make the transfer
    /// Try again later after pending transactions have settled.
    NotEnoughRunes,
...
pub fn build_unsigned_transaction(
    ...
    let runes_inputs_value = runes_utxos_guard.iter().map(|u| u.value).sum::<u64>();

    // debug_assert!(runes_inputs_value >= amount);
    // @dev Runtime validation for production safety (debug_assert! is ignored in release builds)
    if runes_inputs_value < amount {
        return Err(BuildTxError::NotEnoughRunes);
    }
    ...
    let edicts: Vec<Edict> = outputs
        ...
        .map(|(i, (_address, rune_amount))| {
            Edict {
                ...
                // the first recipient is at vout 2 after the minter
                output: (i + 2) as u32,
            }
        })
        .collect();
    ...
    let sats_out = ( output_len - 1 ) as u64 * 330; // @note op_return has a value of 0 btc @syron meta (330sats per rune utxo)
    ...
    let mut gas_fee = 0;
    loop {
        ic_cdk::println!("Building txn with fee {:?} (fee per vbyte is {:?} milisats) & runes_sats_in {:?} & sats_out {:?} & inputs {:?} & outputs {:?}", gas_fee, fee_per_vbyte, runes_sats_in, sats_out, inputs, tx_outputs);
...
fn build_unsigned_txn(
    ...
    // @dev if the change value is dust, the transaction will fail
    // the minter address is P2WPKH thus the dust threshold is 294 sats @review (alpha)
    if value < 294 {
        // return Err(BuildTxError::DustOutput {
        //     address: address.clone(),
        //     amount: value,
        // });
        // @dev instead of returning an error, give dust to miner as gas fee
        value = 0;
    } else {
        // @dev add change utxo back to minter
        outputs.push(TxOut {
            address,
            value
        });
...
pub async fn distribute_kyt_fees() {
    ...
    async fn mint(amount: u64, to: candid::Principal, memo: Memo) -> Result<u64, MintError> {
        // @dev Runtime validation for production safety (debug_assert! is ignored in release builds)
        // debug_assert!(memo.0.len() <= LEDGER_MEMO_SIZE as usize);
        if memo.0.len() > LEDGER_MEMO_SIZE as usize {
            return Err(MintError::CallError(0, format!("Memo size {} exceeds limit {}", memo.0.len(), LEDGER_MEMO_SIZE)));
        }
...
pub fn timer() {
    ...
    // @dev 60 seconds is the minimum interval for the process logic task
    // it aligns better with bitcoin block time (10 minutes)
    // @review (alpha)
    const INTERVAL_PROCESSING: Duration = Duration::from_secs(60);
    ...
    //ic_cdk::println!(">>> Checking task queue. Task found: {:?}", task);

    match task.task_type {
        TaskType::ProcessLogic => {
            ic_cdk::spawn(async {
                //ic_cdk::println!("[ProcessLogic]: Task execution started.");
                let _guard = match crate::guard::TimerLogicGuard::new() {
                    Some(guard) => {
                        //ic_cdk::println!("[ProcessLogic]: Timer guard acquired successfully.");
                        guard
                    },
                    None => {
                        ic_cdk::println!("[ProcessLogic]: FAILED to acquire timer guard. Another task is already running.");
                        return
                    },
                };

                // @note the closure passed to `guard` is executed when the guard is dropped,
                // so 'schedule_after' will be called at that time, not immediately.
                ...
                //ic_cdk::println!("[ProcessLogic]: --> Calling finalize_requests()...");
                finalize_requests().await;
                //ic_cdk::println!("[ProcessLogic]: <-- Finished finalize_requests(). Task completed.");
