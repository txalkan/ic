...
pub async fn get_box_address(args: GetBoxAddressArgs) -> String {
    if args.op == SyronOperation::RedeemBitcoin {
        if let Err(err) = get_siwb_principal(&args.ssi).await {
...
//     fn check_network_and_public_key_result(pk_hex: &str, expected: &str) {
//         assert_eq!(
//             network_and_public_key_to_p2wpkh(&hex::decode(pk_hex).unwrap()),
//             expected,
//             "network: {} pk_hey: {}",
//             network,
//             pk_hex
